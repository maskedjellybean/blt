(function() {
  ...
  .factory('rowsFactory', function($http, $q, preloaderFactory) {
    ...
    service.getRows = function() {
      ...
      service.requestPortfolioData()
      .then(function(data) {
        // Clean up and sort images
        service.sortImagesData(data);

        // Preload all images
        var preload_images_promise = service.preloadImages(allImages);

        // Sort images more and add to data
        data = service.addImagesData(data);
        // Create rows
        rows = service.createRows(data);
    ...
    service.preloadImages = function(allImages) {
      var deferred = $q.defer();
      // Call preloader factory
      preloaderFactory.preloadImages(allImages)
      .then(function() {
        deferred.resolve();
      },
      function() {
    ...
    service.requestPortfolioData = function() {
      var deferred = $q.defer();
      $http({
        method: 'JSONP',
        url: config.target
    ...
    service.sortImagesData = function(data) {
    ...
    service.createRows = function(data) {
      ...
      for (p = 0; p <= config.pieces_count_zero; p++) {
        ...
        var new_row = service.rowSplitter(split_pieces_2_row, 2, config.ppr_2_row_count);
        if (new_row) {
        ...
        new_row = service.rowSplitter(split_pieces_3_row, 3, config.ppr_3_row_count);
        if (new_row) {
    ...
    service.togglesObjCon = function() {
    ...
    service.rowSplitter = function(pieces, pieces_per_row, row_count) {
      ...
      if (new_row.length === pieces_per_row) {
        ...
        for (t = 0; t < pieces_per_row; t++) {
          new_row['toggles'][t] = new service.togglesObjCon();
